...
> access is audited: writing to the G-code file at `psGCodePostProcess` (or any other
> filesystem/network/process action) prompts the user with a Yes/No dialog the first time,
> and the answer is remembered for that plugin. See [Plugin Audit Hook](plugin_audit_hook).
